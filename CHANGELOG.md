...
- `<ProductCard />` catalog tile with `in-stock`, `out-of-stock`, `marketplace-down`, and `image-failed` states. Ships with `<ProductCardSkeleton />`. Image fade-in on cold-load + cache-hit paths.
...
- Theme-safe glassy panels (`bg-card/95` instead of `bg-white/95`) so marketplace-down callouts adapt to dark themes.
